...
describe('Trip flow', () => {
  it('creates trip -> stop -> stop-activity -> budget', async () => {
  ...
  const act = await request(app).post(`/trips/stops/${stopId}/activities`).set('Cookie', cookies).send({ title: 'Louvre Visit', cost: 60, day_offset: 0 });
