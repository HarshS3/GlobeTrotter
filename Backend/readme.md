...
Stop Activities (Simplified & Collaborative Model):
Each activity is created directly under a stop with its own explicit cost (no global catalog / assignment join). Any authenticated user can view and add activities to any stop. Deletion is limited to the activity creator or the trip owner.
- POST /trips/stops/:stopId/activities { title, cost, category?, duration_minutes?, day_offset?, start_time?, notes?, metadata? }
- GET /trips/stops/:stopId/activities (returns creator attribution `created_by` & `creator_email` when available)
- DELETE /trips/stops/:stopId/activities/:activityId (creator or trip owner only)
Attribution: column `created_by` added in migration 008.

Budget now sums `cost` from `stop_activities`.

Deprecated (Removed) Endpoints (migration path only – legacy tables retained temporarily until cleanup):
### Itinerary Endpoint
Aggregated view combining trip core fields, stop counts, activity counts, per-stop activity details and total cost.
	...
	"stops": [
		{
			...
			"activities_count": 3,
			"activities_cost": 150.00,
			"activities": [
				{ "id": 55, "title": "Louvre", "category": null, "cost": 60, "day_offset": 0, "start_time": "10:00:00", "duration_minutes": 120, "notes": null, "created_by": 7, "creator_email": "user@example.com" }
			]
		}
	]
}
```
- /trips/activities (catalog)
- /trips/activities/assignments/* (assignment mapping)
